...
const Staff = () => {
    ...
    useEffect(() => {
        ...
        style.textContent = `
            ...
            .premium-compact-input::placeholder {
    ...
    const { user } = useAuth();
    const selectedCompany = { _id: 'global', name: 'Auto Transport', ownerName: 'Admin' };
    ...
    const fetchAllLeaves = async () => {
        ...
        try {
            const { data } = await axios.get(`/api/admin/leaves/all`);
    ...
    const fetchMonthlyReport = async () => {
        ...
        setIsFetching(true);
        try {
            let url = `/api/admin/staff-attendance`;
            let paymentUrl = `/api/admin/salary-payments`;
            ...
            const [attendanceRes, paymentRes, advancesRes] = await Promise.all([
                ...
                axios.get(`/api/admin/advances?month=${selectedMonth}&year=${selectedYear}&isStaffAdvance=true&t=${Date.now()}`).catch(() => ({ data: [] }))
            ]);
    ...
    const fetchStaffStats = async () => {
        if (!selectedCompany?._id) return;
        try {
            const { data } = await axios.get(`/api/admin/staff-stats`);
            setStaffStats(data);
        } catch (error) {
    ...
    const fetchAttendance = async () => {
        ...
        try {
            ...
            const { data } = await axios.get(`/api/admin/staff-attendance${params}`);
    ...
    const handleStaffClick = async (staff) => {
        ...
        try {
            ...
            const { data } = await axios.get(`/api/admin/staff-attendance?month=${selectedMonth}&year=${selectedYear}&staffId=${staffId}`);
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '30px' }}>
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '8px' }}>
                            <div style={{ width: '48px', height: '48px', background: 'var(--accent)', borderRadius: '16px', display: 'flex', justifyContent: 'center', alignItems: 'center', boxShadow: '0 10px 20px -5px var(--accent-glow)' }}>
                                <Users size={24} color="black" />
                            </div>
                            <div>
                                <h1 style={{ color: 'white', fontSize: '32px', fontWeight: '900', margin: 0, letterSpacing: '-1px' }}>Staff <span style={{ color: 'var(--accent)' }}>Operations</span></h1>
                    ...
                    <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                        ...
                        >
                            <Clock size={18} color="var(--accent)" /> Manual Duty
                        </button>
                        <button
                            onClick={() => {
                                setEditingAdvance(null);
                        ...
                        <div className="stat-card-inner">
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                <div>
                                    ...
                                    <h2 style={{ margin: '8px 0 0 0', fontSize: '32px', fontWeight: '900', color: '#00f0ff' }}>₹{totalPending.toLocaleString()}</h2>
                                </div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '4px' }} className="custom-scrollbar">
                        {[
                            ...
                            <button
                                ...
                                style={{
                                    background: view === tab.id ? 'var(--accent)' : 'rgba(255,255,255,0.03)',
                                        ...
                                        {isFetching ? (
                                            ...
                                            >
                                                <td style={{ padding: '12px 25px', borderTopLeftRadius: '16px', borderBottomLeftRadius: '16px' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                                        <div style={{
                                                            ...
                                                            background: staff.status === 'blocked' ? '#f43f5e' : 'linear-gradient(135deg, var(--accent), #d97706)',
                                                ...
                                                <td style={{ padding: '12px 25px' }}>
                                                    <div style={{ display: 'inline-flex', padding: '4px 12px', borderRadius: '8px', background: 'rgba(0, 240, 255, 0.08)', color: 'var(--accent)', fontSize: '11px', fontWeight: '800', textTransform: 'uppercase' }}>
                                            ...
                                            {isFetching ? (
                                                ...
                                                <tr>
                                                    <td colSpan="5">
                                                        <div style={{ padding: '100px 20px', textAlign: 'center', background: 'rgba(255,255,255,0.01)', borderRadius: '24px', border: '1px dashed rgba(255,255,255,0.05)' }}>
                                                            <History size={48} color="var(--accent)" style={{ opacity: 0.2, marginBottom: '20px' }} />
                                                ...
                                                >
                                                    <td style={{ padding: '20px 25px', borderTopLeftRadius: '20px', borderBottomLeftRadius: '20px' }}>
                                                        <div style={{ fontWeight: '900', color: 'var(--accent)', fontSize: '14px' }}>{formatDateIST(record.date)}</div>
                                                        <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)', fontWeight: '800' }}>{DateTime.fromISO(record.date).toFormat('cccc')}</div>
                                                    </td>
                                                    <td style={{ padding: '20px 25px' }}>
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                                            <div style={{ width: '48px', height: '48px', borderRadius: '14px', background: 'rgba(59, 130, 246, 0.1)', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'var(--accent)', fontWeight: '900', fontSize: '18px', border: '1px solid rgba(59, 130, 246, 0.2)' }}>
                                                            ...
                                                            <div>
                                                                ...
                                                                <div style={{ fontSize: '11px', color: 'var(--accent)', marginTop: '4px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '1px' }}>User: {record.staff?.username || 'SYSTEM'}</div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td style={{ padding: '20px 25px' }}>
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                                            {record.status === 'absent' ? (
                                                                <div style={{ background: 'rgba(0, 240, 255, 0.1)', color: 'var(--accent)', padding: '8px 16px', borderRadius: '12px', fontSize: '12px', fontWeight: '900', letterSpacing: '1px', border: '1px solid rgba(0, 240, 255, 0.2)' }}>
                                                                    ON LEAVE / ABSENT
                                                                </div>
                                                            ) : (
                                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                                    ...
                                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: record.punchOut?.time ? '#f43f5e' : 'var(--accent)', fontSize: '13px', fontWeight: '900' }}>
                                                            ...
                                                            <div style={{ maxWidth: '240px' }}>
                                                                ...
                                                                {record.punchIn?.location?.latitude && (
                                                                    <a
                                                                        ...
                                                                        style={{ fontSize: '10px', color: 'var(--accent)', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '0.5px', marginTop: '4px', display: 'inline-block', textDecoration: 'none', background: 'rgba(0, 240, 255, 0.1)', padding: '2px 8px', borderRadius: '6px' }}
                                                                    >
                                                                        OPEN MAP →
                                                                    </a>
                                                                )}
                                ...
                                <div style={{
                                    ...
                                    <div style={{
                                        ...
                                        <button
                                            ...
                                            style={{
                                                height: '60px', padding: '0 25px', borderRadius: '18px',
                                                background: 'var(--accent)', color: 'black', fontWeight: '800',
                                                    ...
                                                    >
                                                        <td style={{ padding: '12px 25px', borderTopLeftRadius: '16px', borderBottomLeftRadius: '16px' }}>
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                                                <div style={{
                                                                    ...
                                                                    display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '16px', fontWeight: '900', color: 'var(--accent)'
                                                                }}>
                                                                    {item.name.charAt(0)}
                                                                </div>
                                                                <div>
                                                        ...
                                                        <td style={{ padding: '12px 25px' }}>
                                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                                                <div style={{ fontSize: '13px', fontWeight: '900', color: 'var(--accent)' }}>
                                                        ...
                                                        <td style={{ padding: '12px 25px', textAlign: 'right' }}>
                                                            ...
                                                            <div style={{ fontSize: '9px', color: 'var(--accent)', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '1px' }}>Earned So Far</div>
                                                        </td>
                                                        <td style={{ padding: '12px 25px' }}>
                                                            {(() => {
                                                                const payment = salaryPayments.find(p => p.staff === (item.staffId || item._id));
                                                                ...
                                                                return (
                                                                    <div
                                                                        ...
                                                                        style={{
                                                                            ...
                                                                            padding: '6px 12px',
                                                                            borderRadius: '10px',
                                                                            background: isPaid ? 'rgba(16, 185, 129, 0.1)' : 'rgba(0, 240, 255, 0.1)',
                                                                            border: `1px solid ${isPaid ? 'rgba(16, 185, 129, 0.2)' : 'rgba(0, 240, 255, 0.2)'}`,
                                                                            width: 'fit-content'
                                                                        }}
                                                                    >
                                                                        <div style={{
                                                                            ...
                                                                            height: '8px',
                                                                            borderRadius: '50%',
                                                                            background: isPaid ? '#10b981' : '#00f0ff',
                                                                            boxShadow: `0 0 10px ${isPaid ? 'rgba(16, 185, 129, 0.5)' : 'rgba(0, 240, 255, 0.5)'}`
                                                                        }}></div>
                                                                        <span style={{ fontSize: '11px', fontWeight: '900', color: isPaid ? '#10b981' : '#00f0ff' }}>
                                                                            {isPaid ? 'PAID' : 'DUE'}
                                                                        </span>
                                                                    </div>
                                                                );
                                                        ...
                                                        <td style={{ padding: '12px 25px', borderTopRightRadius: '16px', borderBottomRightRadius: '16px', textAlign: 'right' }}>
                                                            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                                                                <motion.button
                                                                    whileHover={{ scale: 1.1, background: 'rgba(0, 240, 255, 0.2)' }}
                                                                    whileTap={{ scale: 0.95 }}
                                                                    onClick={() => downloadSalarySlip(item)}
                                                                    style={{ width: '38px', height: '38px', borderRadius: '12px', background: 'rgba(0, 240, 255, 0.05)', color: 'var(--accent)', border: '1px solid rgba(0, 240, 255, 0.2)', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center' }}
                                                                    title="Download Slip"
                                                                >
                                                                    <FileText size={16} />
                                                                </motion.button>
                ...
                <AnimatePresence>
                    {showAddModal && (
                        <div style={{
                            ...
                            <div style={{ position: 'absolute', top: '10%', right: '10%', width: '400px', height: '400px', background: 'rgba(0, 240, 255, 0.05)', filter: 'blur(100px)', borderRadius: '50%', zIndex: -1 }}></div>
                            ...
                            <motion.div
                                ...
                                style={{
                                    ...
                                    boxShadow: '0 50px 100px -20px rgba(0,0,0,0.8), 0 0 50px -10px var(--accent-glow)'
                                }}
                            >
                                <div style={{ padding: '30px 50px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                                        <div style={{ width: '64px', height: '64px', background: 'var(--accent)', borderRadius: '20px', display: 'flex', justifyContent: 'center', alignItems: 'center', boxShadow: '0 10px 20px -5px var(--accent-glow)' }}>
                                ...
                                <form onSubmit={handleAddStaff} style={{ padding: '40px 50px', display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '40px', overflowY: 'auto' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                                        <section style={{ background: 'rgba(255,255,255,0.02)', padding: '30px', borderRadius: '30px', border: '1px solid rgba(255,255,255,0.05)', position: 'relative', overflow: 'hidden' }}>
                                            <div style={{ position: 'absolute', top: 0, right: 0, width: '150px', height: '150px', background: 'radial-gradient(circle, var(--accent-glow) 0%, transparent 70%)', opacity: 0.1 }}></div>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                                    <label style={{ fontSize: '10px', fontWeight: '800', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '1px' }}>Employment Type</label>
                                                    
                                                    {/* Regular Card */}
                                                    <div onClick={() => setFormData({ ...formData, staffType: 'Regular' })} style={{ display: 'flex', alignItems: 'center', gap: '15px', padding: '15px 20px', borderRadius: '16px', border: formData.staffType === 'Regular' ? '1px solid var(--accent)' : '1px solid rgba(255,255,255,0.05)', background: formData.staffType === 'Regular' ? 'rgba(251,191,36,0.05)' : 'rgba(255,255,255,0.02)', cursor: 'pointer', transition: '0.3s' }}>
                                                        <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: formData.staffType === 'Regular' ? 'rgba(251,191,36,0.1)' : 'rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                                            <ShieldCheck size={18} color={formData.staffType === 'Regular' ? 'var(--accent)' : 'rgba(255,255,255,0.5)'} />
                                                        </div>
                                                        <div>
                                                            <h4 style={{ margin: 0, fontSize: '14px', fontWeight: '800', color: formData.staffType === 'Regular' ? 'white' : 'rgba(255,255,255,0.7)' }}>Regular (With Leaves)</h4>
                                                            <p style={{ margin: '4px 0 0 0', fontSize: '11px', fontWeight: '600', color: 'rgba(255,255,255,0.4)' }}>Standard payroll with monthly leave quota tracking</p>
                                                        </div>
                                                    </div>

                                                    {/* Fixed Card */}
                                                    <div onClick={() => setFormData({ ...formData, staffType: 'Fixed' })} style={{ display: 'flex', alignItems: 'center', gap: '15px', padding: '15px 20px', borderRadius: '16px', border: formData.staffType === 'Fixed' ? '1px solid var(--accent)' : '1px solid rgba(255,255,255,0.05)', background: formData.staffType === 'Fixed' ? 'rgba(251,191,36,0.05)' : 'rgba(255,255,255,0.02)', cursor: 'pointer', transition: '0.3s' }}>
                                                        <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: formData.staffType === 'Fixed' ? 'rgba(251,191,36,0.1)' : 'rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                                            <Clock size={18} color={formData.staffType === 'Fixed' ? 'var(--accent)' : 'rgba(255,255,255,0.5)'} />
                                                        </div>
                                                        <div>
                                                            <h4 style={{ margin: 0, fontSize: '14px', fontWeight: '800', color: formData.staffType === 'Fixed' ? 'white' : 'rgba(255,255,255,0.7)' }}>Fixed Wage</h4>
                                                            <p style={{ margin: '4px 0 0 0', fontSize: '11px', fontWeight: '600', color: 'rgba(255,255,255,0.4)' }}>Fixed monthly salary without leave allowance deduction</p>
                                                        </div>
                                                    </div>

                                                    {/* Daily Card */}
                                                    <div onClick={() => setFormData({ ...formData, staffType: 'Daily' })} style={{ display: 'flex', alignItems: 'center', gap: '15px', padding: '15px 20px', borderRadius: '16px', border: formData.staffType === 'Daily' ? '1px solid var(--accent)' : '1px solid rgba(255,255,255,0.05)', background: formData.staffType === 'Daily' ? 'rgba(251,191,36,0.05)' : 'rgba(255,255,255,0.02)', cursor: 'pointer', transition: '0.3s' }}>
                                                        <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: formData.staffType === 'Daily' ? 'rgba(251,191,36,0.1)' : 'rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                                            <IndianRupee size={18} color={formData.staffType === 'Daily' ? 'var(--accent)' : 'rgba(255,255,255,0.5)'} />
                                                        </div>
                                                        <div>
                                                            <h4 style={{ margin: 0, fontSize: '14px', fontWeight: '800', color: formData.staffType === 'Daily' ? 'white' : 'rgba(255,255,255,0.7)' }}>Daily Wage</h4>
                                                            <p style={{ margin: '4px 0 0 0', fontSize: '11px', fontWeight: '600', color: 'rgba(255,255,255,0.4)' }}>Day-based wage calculated strictly from presence logs</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '25px' }}>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                        <label style={{ fontSize: '10px', fontWeight: '800', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '1px' }}>Monthly Salary</label>
                                                        <div style={{ position: 'relative' }}>
                                                            <span style={{ position: 'absolute', left: '18px', top: '50%', transform: 'translateY(-50%)', color: 'var(--accent)', fontWeight: '900', fontSize: '18px' }}>₹</span>
                                                    ...
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                        <label style={{ fontSize: '10px', fontWeight: '800', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '1px' }}>Joining Date</label>
                                                        <div style={{ position: 'relative' }}>
                                                            <PremiumDateInput
                                                                ...
                                                                inputStyle={{ fontSize: '20px', fontWeight: '1000', color: 'var(--accent)' }}
                                                            />
                                                        </div>
                                                    </div>
                                                </div>

                                                {formData.staffType === 'Regular' && (
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                        <label style={{ fontSize: '10px', fontWeight: '800', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '1px' }}>Monthly Leave Quota</label>
                                                        <div style={{ position: 'relative' }}>
                                                            <span style={{ position: 'absolute', left: '18px', top: '50%', transform: 'translateY(-50%)', color: 'rgba(255,255,255,0.3)' }}>
                                                                <Calendar size={18} />
                                                            </span>
                                                            <input required type="number" className="premium-compact-input" placeholder="e.g. 4" style={{ width: '100%', height: '56px', padding: '0 20px 0 45px', fontSize: '16px', fontWeight: '900', borderRadius: '16px', color: 'white', background: 'rgba(255,255,255,0.03)' }} value={formData.monthlyLeaveAllowance} onChange={(e) => setFormData({ ...formData, monthlyLeaveAllowance: e.target.value })} />
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                                            {formData.staffType !== 'Fixed' && (
                                                <div style={{
                                                    ...
                                                    <p style={{ margin: 0, fontSize: '11px', color: 'rgba(255,255,255,0.6)', fontWeight: '600', lineHeight: '1.5' }}>
                                                        <span style={{ color: 'var(--accent)', fontWeight: '800' }}>POLICY:</span> Unused leaves from the quota will automatically carry forward to the next month for this personnel.
                                                    </p>
                                                </div>
                                            )}
                                        </section>
                                        ...
                                        <div style={{ marginTop: '10px' }}>
                                            <motion.button
                                                ...
                                                type="submit"
                                                style={{
                                                    width: '100%', height: '70px', background: 'var(--accent)', color: 'black',
                                                    border: 'none', borderRadius: '24px', fontSize: '18px', fontWeight: '1000',
                                                    cursor: 'pointer', boxShadow: '0 20px 40px -10px var(--accent-glow)', letterSpacing: '1px',
                        ...
                        <div style={{
                            ...
                            >
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', height: '100%', overflow: 'hidden', position: 'relative' }}>
                                    <div style={{ position: 'absolute', top: '-20%', left: '-10%', width: '30%', height: '30%', background: 'radial-gradient(circle, rgba(0, 240, 255, 0.15) 0%, transparent 70%)', filter: 'blur(80px)', pointerEvents: 'none' }}></div>
                                    ...
                                    <div style={{ background: 'rgba(255,255,255,0.02)', borderRight: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', backdropFilter: 'blur(20px)', zIndex: 2, overflowY: 'auto' }}>
                                        <div style={{ padding: '30px 25px 20px 25px', borderBottom: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                                                <div style={{ width: '54px', height: '54px', borderRadius: '18px', background: 'linear-gradient(135deg, rgba(0, 240, 255, 0.2), transparent)', border: '1px solid rgba(0, 240, 255, 0.3)', display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '22px', fontWeight: '800', color: 'var(--accent)', flexShrink: 0 }}>
                                                ...
                                                <div>
                                                    ...
                                                    <p style={{ margin: '3px 0 0 0', fontSize: '10px', color: 'var(--accent)', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '1px' }}>{selectedStaffReport.designation || 'Staff Member'}</p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div style={{
                                            ...
                                            <div style={{ position: 'absolute', top: '-50%', left: '-50%', width: '200%', height: '200%', background: 'radial-gradient(circle, rgba(0, 240, 255, 0.1) 0%, transparent 50%)', opacity: 0.5 }}></div>
                                            <p style={{ margin: 0, fontSize: '9px', fontWeight: '700', color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '2px' }}>EARNED SO FAR</p>
                                            <h1 style={{ margin: '6px 0 0 0', fontSize: '40px', fontWeight: '900', color: 'white', letterSpacing: '-2px', textShadow: '0 0 30px rgba(0, 240, 255, 0.4)', position: 'relative' }}>
                                                <span style={{ fontSize: '14px', verticalAlign: 'top', color: 'var(--accent)', marginRight: '3px', fontWeight: '700' }}>₹</span>
                                            ...
                                            <button
                                                ...
                                                style={{
                                                    ...
                                                    padding: '16px',
                                                    borderRadius: '18px',
                                                    background: isGeneratingPDF ? 'rgba(255,255,255,0.05)' : 'rgba(0, 240, 255, 0.1)',
                                                    border: '1px solid rgba(0, 240, 255, 0.3)',
                                                    color: 'var(--accent)',
                                                    fontSize: '12px',
                                                    fontWeight: '900',
                                        ...
                                        <div style={{ padding: '12px 16px 24px 16px', flexGrow: 1 }}>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(14,165,233,0.07)', padding: '10px 12px', borderRadius: '12px', border: '1px solid rgba(14,165,233,0.14)' }}>
                                                    <div>
                                                        <p style={{ margin: 0, fontSize: '10px', color: 'var(--accent)', fontWeight: '700' }}>🎫 TOTAL LEAVES TAKEN</p>
                                                        <p style={{ margin: '2px 0 0 0', fontSize: '9px', color: 'rgba(14,165,233,0.5)', fontWeight: '500' }}>All leaves are unpaid as per policy</p>
                                                    </div>
                                                    <span style={{ fontWeight: '800', color: 'var(--accent)', fontSize: '13px' }}>{selectedStaffReport.leavesTaken || 0} <small style={{ fontSize: '9px', opacity: 0.6 }}>days</small></span>
                                                </div>

                                                {/* Sunday Holidays (Paid) */}
                                                {selectedStaffReport.staffType !== 'Daily' && (selectedStaffReport.sundaysPassed || 0) > 0 && (
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0, 240, 255, 0.05)', padding: '10px 12px', borderRadius: '12px', border: '1px solid rgba(0, 240, 255, 0.1)' }}>
                                                        <div>
                                                            <p style={{ margin: 0, fontSize: '10px', color: 'var(--accent)', fontWeight: '700' }}>⭐ SUNDAY HOLIDAYS</p>
                                                            <p style={{ margin: '2px 0 0 0', fontSize: '9px', color: 'rgba(0, 240, 255, 0.5)', fontWeight: '500' }}>Automatically paid rest days</p>
                                                        </div>
                                                        <span style={{ fontWeight: '800', color: 'var(--accent)', fontSize: '13px' }}>{selectedStaffReport.sundaysPassed} <small style={{ fontSize: '9px', opacity: 0.6 }}>days</small></span>
                                                    </div>
                                                )}
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)', padding: '10px 12px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.04)' }}>
                                                    <div>
                                                        <p style={{ margin: 0, fontSize: '10px', color: 'rgba(255,255,255,0.4)', fontWeight: '700' }}>❌ UNPAID ABSENT DAYS</p>
                                                        <p style={{ margin: '2px 0 0 0', fontSize: '9px', color: 'rgba(255,255,255,0.2)', fontWeight: '500' }}>Not applicable for {selectedStaffReport.staffType} wage</p>
                                                    </div>
                                                    <div style={{ textAlign: 'right' }}>
                                                        <p style={{ margin: 0, fontWeight: '800', color: 'rgba(255,255,255,0.3)', fontSize: '13px' }}>-</p>
                                                    </div>
                                                </div>
                                                )}
                                                ...
                                                <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '16px', padding: '15px', border: '1px solid rgba(255,255,255,0.08)' }}>
                                                    ...
                                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                                        <div>
                                                            <div style={{ fontSize: '16px', fontWeight: '900', color: '#818cf8' }}>{selectedStaffReport.totalLeaveAccrued || 0}</div>
                                                            <div style={{ fontSize: '9px', fontWeight: '700', color: 'rgba(255,255,255,0.3)', marginTop: '2px' }}>PREVIOUS C/F</div>
                                                        </div>
                                                        <div>
                                                            <div style={{ fontSize: '16px', fontWeight: '900', color: 'white' }}>{selectedStaffReport.monthlyLeaveAllowance || 4}</div>
                                                            <div style={{ fontSize: '9px', fontWeight: '700', color: 'rgba(255,255,255,0.3)', marginTop: '2px' }}>ALLOWED LEAVES</div>
                                                        </div>
                                                        <div>
                                                            <div style={{ fontSize: '16px', fontWeight: '900', color: '#10b981' }}>{(selectedStaffReport.totalLeaveAccrued || 0) + (selectedStaffReport.monthlyLeaveAllowance || 4)}</div>
                                                            <div style={{ fontSize: '9px', fontWeight: '700', color: 'rgba(255,255,255,0.3)', marginTop: '2px' }}>TOTAL AVAILABLE</div>
                                                        </div>
                                                        <div>
                                                            <div style={{ fontSize: '16px', fontWeight: '900', color: '#f43f5e' }}>{selectedStaffReport.totalLeaveUsed || 0}</div>
                                                            <div style={{ fontSize: '9px', fontWeight: '700', color: 'rgba(255,255,255,0.3)', marginTop: '2px' }}>UTILIZED THIS MONTH</div>
                                                        </div>
                                                    </div>
                                                </div>
                                                )}
                                                ...
                                                <div style={{ background: 'linear-gradient(135deg, rgba(251,191,36,0.1), rgba(251,191,36,0.03))', border: '1px solid rgba(251,191,36,0.25)', borderRadius: '14px', padding: '12px 14px', marginTop: '2px' }}>
                                                    <p style={{ margin: 0, fontSize: '9px', color: 'var(--accent)', fontWeight: '700', letterSpacing: '1px' }}>🧮 FINAL PAYOUT CALCULATION</p>
                                                    ...
                                                    <p style={{ margin: '5px 0 0 0', fontSize: '15px', fontWeight: '900', color: 'var(--accent)' }}>= ₹{(selectedStaffReport.finalSalary || 0).toLocaleString()} (TOTAL SALARY)</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div style={{ display: 'flex', gap: '10px' }}>
                                                ...
                                                {isFetching && (
                                                    <div style={{ width: '36px', height: '36px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                                        <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: 'linear' }}><Settings size={16} color="var(--accent)" /></motion.div>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '14px', fontSize: '11px', fontWeight: '600', marginRight: '40px', flexWrap: 'wrap' }}>
                                                    ...
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '5px', color: 'var(--accent)' }}>
                                                        <div style={{ width: '10px', height: '10px', background: 'var(--accent)', borderRadius: '50%', boxShadow: '0 0 10px rgba(14, 165, 233, 0.5)' }}></div> HALF DAY
                                                    </div>
                                            ...
                                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '10px' }}>
                                                ...
                                                {(selectedStaffReport.attendanceData || []).map((attendanceRecord, idx) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            {isSunday && <div style={{ position: 'absolute', top: '2px', right: '2px', width: '4px', height: '4px', background: 'var(--accent)', borderRadius: '50%' }}></div>}
                                                            <span style={{
                                                                fontSize: '12px',
                                                                fontWeight: '800',
                                                                color: isPresent ? '#10b981' : isHalfDay ? 'var(--accent)' : isAbsent ? '#f43f5e' : 'rgba(255,255,255,0.3)',
                                                                display: 'flex',
                                                                flexDirection: 'column',
                                                                alignItems: 'center',
                                                                lineHeight: '1',
                                                            ...
                                                            {isPresent ? (
                                                                ...
                                                                <div style={{ width: '8px', height: '8px', background: 'var(--accent)', borderRadius: '50%', boxShadow: '0 0 12px var(--accent)' }}></div>
                                                            ) : isAbsent ? (
                                        ...
                                        <div style={{
                                            ...
                                            <div>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                                                    ...
                                                    <span style={{ fontSize: '11px', fontWeight: '700', color: 'var(--accent)' }}>{Math.round((selectedStaffReport.finalSalary / (selectedStaffReport.baseSalary || selectedStaffReport.salary || 1)) * 100)}%</span>
                                                </div>
                                                <div style={{ height: '8px', background: 'rgba(255,255,255,0.05)', borderRadius: '4px', overflow: 'hidden' }}>
                                                    <motion.div
                                                        ...
                                                        style={{ height: '100%', background: 'linear-gradient(90deg, var(--accent), var(--accent))', boxShadow: '0 0 15px rgba(0, 240, 255, 0.3)' }}
                                                    />
                                                </div>
                                            </div>
                                            <div>
                                                                ...
                                                                <tr key={log._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                                                                    ...
                                                                    <td style={{ padding: '15px 20px' }}>
                                                                        <span style={{
                                                                            ...
                                                                            padding: '4px 8px',
                                                                            borderRadius: '6px',
                                                                            background: log.statusLabel === 'WEEKLY OFF' ? 'rgba(0, 240, 255, 0.15)' : (log.status === 'present' ? 'rgba(34, 197, 94, 0.15)' : 'rgba(239, 68, 68, 0.1)'),
                                                                            color: log.statusLabel === 'WEEKLY OFF' ? 'var(--accent)' : (log.status === 'present' ? '#22c55e' : '#ef4444')
                                                                        }}>
                    ...
                    {showBackdateModal && (
                        <div style={{
                            ...
                            >
                                ...
                                <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '6px', background: 'linear-gradient(90deg, var(--accent), var(--accent), var(--accent))', backgroundSize: '200% 100%', animation: 'gradientMove 3s infinite linear' }}></div>
                                    ...
                                    <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                                        ...
                                        <div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                                <span style={{ fontSize: '10px', fontWeight: '800', color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '2px' }}>Security Panel</span>
                                ...
                                <form onSubmit={handleBackdateSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
                                    <motion.div
                                        ...
                                        style={{ background: 'rgba(0, 240, 255, 0.08)', border: '1px solid rgba(0, 240, 255, 0.2)', borderRadius: '20px', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '14px' }}
                                    >
                                        <div style={{ width: '32px', height: '32px', borderRadius: '10px', background: 'rgba(0, 240, 255, 0.1)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                            <AlertCircle size={18} color="var(--accent)" />
                                        </div>
                                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                                            <span style={{ fontSize: '12px', color: 'var(--accent)', fontWeight: '800', letterSpacing: '0.2px' }}>Attendance Policy</span>
                                            <span style={{ fontSize: '11px', color: 'rgba(0, 240, 255, 0.6)', fontWeight: '600' }}>Backdated entries are limited to the previous 60 days (2 Months).</span>
                                        </div>
                                    </motion.div>
                                        ...
                                        <div className="premium-input-group">
                                            ...
                                            <select
                                                ...
                                                style={{
                                                    ...
                                                    paddingLeft: '20px',
                                                    fontSize: '15px',
                                                    color: backdateForm.status === 'present' ? '#10b981' : backdateForm.status === 'half-day' ? 'var(--accent)' : '#f43f5e',
                                                    fontWeight: '700'
                                                }}
                                            >
                                                <option value="present" style={{ background: '#0B1121', color: '#10b981' }}>PRESENT</option>
                                                <option value="half-day" style={{ background: '#0B1121', color: 'var(--accent)' }}>HALF DAY</option>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                                    <div>
                                        <h2 style={{ fontSize: '24px', fontWeight: '900', color: 'white', margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            <IndianRupee color="var(--accent)" /> {paymentModal.isPaid ? 'Edit Payment' : 'Mark as Paid'}
                                        </h2>
