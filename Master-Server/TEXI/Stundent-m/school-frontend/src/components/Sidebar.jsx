    ...
    return (
        ...
        >
                ...
                <Link to="/admin" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '15px' }}>
                    ...
                    <div style={{ overflow: 'hidden' }}>
                        <h1 style={{
                            ...
                            School<span style={{ color: theme.primary }}>Transport</span>
                        </h1>
                        <p style={{
                            fontSize: '10px',
            ...
            <nav style={{ flex: 1, overflowY: 'auto', paddingRight: '5px' }} className="sidebar-nav-scroll">
                ...
                <NavGroup title="School Operations" labelKey="school_operations" icon={Users} isOpen={openGroups.school} onToggle={() => toggleGroup('school')}>
                    <NavItem item={{ path: '/admin/students', label: 'Students', labelKey: 'students' }} onClick={onClose} isSubItem />
                    <NavItem item={{ path: '/admin/routes', label: 'Routes & Stops', labelKey: 'routes' }} onClick={onClose} isSubItem />
                    <NavItem item={{ path: '/admin/bus-seating', label: 'Bus Seating Layout', labelKey: 'bus_seating' }} onClick={onClose} isSubItem />
                </NavGroup>



                {hasAccess('driversService') && (
                    <NavGroup title="Driver Management" labelKey="drivers_services" icon={Users} isOpen={openGroups.drivers} onToggle={() => toggleGroup('drivers')}>
                        {hasAccess('driversService', 'drivers') && <NavItem item={{ path: '/admin/drivers-panel', label: 'Bus Drivers', labelKey: 'drivers' }} onClick={onClose} isSubItem />}
                    </NavGroup>
                )}

                {hasAccess('fleetOperations') && (
                    <NavGroup title="Fuel Operations" labelKey="fleet_operations" icon={Fuel} isOpen={openGroups.vehicles} onToggle={() => toggleGroup('vehicles')}>
                        {hasAccess('fleetOperations', 'fuel') && <NavItem item={{ path: '/admin/fuel', label: 'Fuel Logs', labelKey: 'fuel' }} onClick={onClose} isSubItem />}
                    </NavGroup>
                )}
                {hasAccess('vehiclesManagement') && (
                    <NavGroup title="Vehicle Maintenance" labelKey="vehicles_life" icon={Wrench} isOpen={openGroups.maintenance} onToggle={() => toggleGroup('maintenance')}>
                        ...
                        {hasAccess('vehiclesManagement', 'carLogs') && <NavItem item={{ path: '/admin/vehicle-month-details', label: 'Vehicle Logs', labelKey: 'car_logs' }} onClick={onClose} isSubItem />}
                        {hasAccess('vehiclesManagement', 'vehiclesMgt') && <NavItem item={{ path: '/admin/vehicles', label: 'Vehicles Inventory', labelKey: 'vehicles_mgt' }} onClick={onClose} isSubItem />}
                    </NavGroup>
                )}
