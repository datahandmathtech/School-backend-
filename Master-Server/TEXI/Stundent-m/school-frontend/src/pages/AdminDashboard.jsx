...
const AdminDashboard = () => {
    ...
    const [aiPredictiveAlerts, setAiPredictiveAlerts] = useState([]);
    ...
    const [chatHistory, setChatHistory] = useState([{ type: 'ai', text: 'Hello! I am your AI Fleet Assistant. How can I help you today?' }]);
    ...
    const fetchStats = async () => {
        ...
        try {
            ...
            // Fetch AI Predictive Alerts
            try {
                const aiRes = await axios.get(`/api/admin/ai-maintenance/${selectedCompany._id}`, {
                    headers: { Authorization: `Bearer ${userInfo.token}` }
                });
                setAiPredictiveAlerts(aiRes.data || []);
            } catch(e) { console.error('Error fetching AI alerts', e); }
    ...
    const handleAskChatbot = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(`/api/admin/ai-chat/${selectedCompany._id}`, { query: chatQuery }, {
                headers: { Authorization: `Bearer ${userInfo.token}` }
            });
            setChatHistory([...newHistory, { type: 'ai', text: data.answer }]);
        } catch (error) {
            setChatHistory([...newHistory, { type: 'ai', text: 'Error connecting to AI Assistant.' }]);
        } finally {
    ...
    const renderAlertCounts = (category) => {
        ...
        if (category === 'AIPredictive') {
            return aiPredictiveAlerts?.length || 0;
        }
                    ...
                    <div style={{ marginBottom: 'clamp(20px, 4vw, 30px)', marginTop: '30px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                            {[
                                ...
                                { id: 'AIPredictive', label: 'AI Prediction', color: '#fbbf24', icon: Activity, desc: 'Smart AI Alerts' }
                            ].map(btn => {
                                ...
                                return (
                                    <div key={btn.id} 
                                         ...
                                         style={{
                                             flex: '1 1 calc(16.6% - 12px)', minWidth: '160px',
                                ...
                                >
                                    <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)' }}>
                                        <h2 style={{ color: 'white', margin: 0, fontSize: '18px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            ...
                                            {activeAlertModal === 'AIPredictive' && <><Activity color="#fbbf24" /> AI Predictive Alerts</>}
                                        </h2>
                                            ...
                                            <div className="expiry-alerts-grid">
                                                {aiPredictiveAlerts.map((alert, index) => (
                                                    <div
                                                        ...
                                                        style={{
                                                            background: alert.type === 'Critical' ? 'rgba(244, 63, 94, 0.1)' : 'rgba(251, 191, 36, 0.1)',
                                                            border: `1px solid ${alert.type === 'Critical' ? 'rgba(244, 63, 94, 0.3)' : 'rgba(251, 191, 36, 0.3)'}`,
                                                            position: 'relative',
                                                            overflow: 'hidden'
                                                        }}
                                                    >
                                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px', gap: '8px' }}>
                                                            ...
                                                            <span style={{ fontSize: 'clamp(9px, 2vw, 10px)', padding: '4px 8px', borderRadius: '6px', background: alert.type === 'Critical' ? '#f43f5e' : '#fbbf24', color: alert.type === 'Critical' ? 'white' : 'black', fontWeight: '800', flexShrink: 0 }}>
                                                                {alert.type}
                                                            </span>
                                                        </div>
                                                        <div className="alert-type" style={{ color: 'white', marginBottom: '8px', fontSize: '13px' }}>
                                                            {alert.message}
                                                        </div>
            ...
            <div style={{ position: 'fixed', bottom: '20px', right: '20px', zIndex: 10000, display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                <AnimatePresence>
                    {isChatOpen && (
                        ...
                        >
                            <div style={{ padding: '15px', background: 'rgba(16, 185, 129, 0.1)', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <Activity color="#10b981" size={18} />
                                    <h3 style={{ margin: 0, fontSize: '14px', fontWeight: '800', color: 'white' }}>AI Fleet Assistant</h3>
                                </div>
                            ...
                            <div style={{ flex: 1, padding: '15px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                {chatHistory.map((msg, idx) => (
                                    <div key={idx} style={{ alignSelf: msg.type === 'user' ? 'flex-end' : 'flex-start', background: msg.type === 'user' ? 'rgba(56, 189, 248, 0.1)' : 'rgba(255,255,255,0.05)', border: `1px solid ${msg.type === 'user' ? 'rgba(56, 189, 248, 0.2)' : 'rgba(255,255,255,0.1)'}`, padding: '10px 12px', borderRadius: '12px', maxWidth: '85%', fontSize: '12px', lineHeight: '1.4', color: 'rgba(255,255,255,0.9)' }}>
                                        {msg.text}
                                    </div>
                                ))}
                                {chatLoading && (
                                    <div style={{ alignSelf: 'flex-start', color: 'rgba(255,255,255,0.5)', fontSize: '11px' }}>AI is thinking...</div>
                                )}
                            </div>
                            <form onSubmit={handleAskChatbot} style={{ padding: '10px', background: 'rgba(0,0,0,0.2)', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: '8px' }}>
                                <input
                                    type="text"
                                    value={chatQuery}
                                    onChange={(e) => setChatQuery(e.target.value)}
                                    placeholder="Ask anything..."
                                    style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', padding: '8px 12px', borderRadius: '8px', color: 'white', fontSize: '12px', outline: 'none' }}
                                />
                                <button type="submit" disabled={chatLoading} style={{ background: '#10b981', color: 'white', border: 'none', padding: '0 12px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>Send</button>
                            </form>
                        </motion.div>
                    )}
                </AnimatePresence>
                ...
                <button
                    ...
                    style={{
                        width: '56px', height: '56px', borderRadius: '50%', background: 'linear-gradient(135deg, #10b981, #059669)', border: 'none', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 15px rgba(16, 185, 129, 0.4)'
                    }}
                >
                    {isChatOpen ? <X size={24} /> : <Activity size={24} />}
                </button>
            </div>
        </div>
    );
