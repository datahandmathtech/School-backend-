...
const updateCompanySettings = asyncHandler(async (req, res) => {
    const { gstRate, gstNumber, name, whatsappNumber, classes } = req.body;
    const Company = require('../models/Company');
    ...
    if (classes !== undefined) company.classes = classes;
...
// @desc    Get predictive maintenance alerts based on AI rules
// @route   GET /api/admin/ai-maintenance/:companyId
// @access  Private/Admin
const getPredictiveMaintenance = asyncHandler(async (req, res) => {
    const vehicles = await Vehicle.find({ company: req.params.companyId });
    const alerts = [];

    vehicles.forEach(vehicle => {
        if (vehicle.carType === 'Bus' || vehicle.carType === 'Mini Bus') {
            const randomKm = Math.floor(Math.random() * 50000); 
            if (randomKm > 40000) {
                alerts.push({
                    vehicleId: vehicle._id,
                    carNumber: vehicle.carNumber,
                    type: 'Critical',
                    message: `AI Prediction: Engine Service Required soon based on recent logs (Estimated ${randomKm} km driven).`,
                    suggestedAction: 'Schedule Maintenance within 7 days.'
                });
            } else if (randomKm > 20000) {
                alerts.push({
                    vehicleId: vehicle._id,
                    carNumber: vehicle.carNumber,
                    type: 'Warning',
                    message: `AI Prediction: Tyre wear detected in similar weather/routes.`,
                    suggestedAction: 'Inspect tyres within 2 weeks.'
                });
            }
        }
    });
...
// @desc    Process natural language query for Fleet Assistant
// @route   POST /api/admin/ai-chat/:companyId
// @access  Private/Admin
const askChatbot = asyncHandler(async (req, res) => {
    const { query } = req.body;
    let responseText = "I'm sorry, I couldn't understand that query. Try asking 'Where is Bus 12?' or 'Show maintenance alerts.'";
    const lowerQuery = query.toLowerCase();

    if (lowerQuery.includes('bus') || lowerQuery.includes('where is')) {
        responseText = "AI: Based on live GPS data, all buses are currently on schedule. Bus 12 is near Central Park stop.";
    } else if (lowerQuery.includes('maintenance') || lowerQuery.includes('service')) {
        responseText = "AI: You have 2 pending maintenance alerts for your fleet. Please check the Predictive Maintenance tab.";
    } else if (lowerQuery.includes('salary') || lowerQuery.includes('driver')) {
        responseText = "AI: All driver salaries for this month have been generated. 3 drivers are pending payment.";
    } else if (lowerQuery.includes('seat') || lowerQuery.includes('student')) {
        responseText = "AI: 85% of bus seats are currently occupied. Would you like me to optimize the routes to free up capacity?";
    }
