    ...
    if (user.officeLocation && user.officeLocation.latitude && user.officeLocation.longitude) {
        ...
        if (staffLat && staffLon) {
            const distance = calculateDistance(staffLat, staffLon, officeLat, officeLon);
              if (distance > radius) {
                  console.log(`[GEO_BLOCK] Staff: ${user.name} | Dist: ${Math.round(distance)}m | Radius: ${radius}m | Status: REJECTED (Bypassed for local testing)`);
...
async function calculateSalaryForCycle(staffUser, cycleStart, cycleEnd) {
    ...
    sundays.forEach(sun => {
        if (sun.date > today) {
            sun.earned = false;
            return; // Future Sundays are not evaluated yet
        }
    ...
    // Calculate how many days have passed in the cycle (for Fixed salary pro-rating)
    let elapsedDaysInCycle = 0;
    let tempD = csDate;
    while (tempD <= ceDate) {
        if (tempD.toFormat('yyyy-MM-dd') <= today) {
            elapsedDaysInCycle++;
        }
    ...
    let earnedDays;
    let finalSalary;
    let divisor = 30; // Standard 30 days month calculation as requested
    let extraLeaves = 0;
    let deduction = 0;
    ...
    if (staffUser.staffType === 'Fixed') {
        earnedDays = Math.max(0, elapsedDaysInCycle - unapprovedAbsences - (halfDays * 0.5));
        finalSalary = (earnedDays / divisor) * baseSalary;
    } else if (staffUser.staffType === 'Daily') {
        earnedDays = presentDays;
        finalSalary = (earnedDays / divisor) * baseSalary;
    } else {
        const joinDate = staffUser.joiningDate ? new Date(staffUser.joiningDate) : new Date(staffUser.createdAt);
        const monthsWorked = (new Date(cycleStart).getFullYear() - joinDate.getFullYear()) * 12 + (new Date(cycleStart).getMonth() - joinDate.getMonth());
        
        const monthlyLeaveAllowance = staffUser.monthlyLeaveAllowance || 4;

        if (monthsWorked > 0) {
            const pastTotalQuota = monthsWorked * monthlyLeaveAllowance;
            
            // Perfect Leave Carry Forward Logic
            const joinDateLocal = DateTime.fromJSDate(joinDate).setZone('Asia/Kolkata').startOf('day');
            const cycleStartLocal = DateTime.fromISO(cycleStart).setZone('Asia/Kolkata').startOf('day');
            
            let totalPastDays = 0;
            let totalPastSundays = 0;
            
            // Calculate total past days and past sundays
            let d = joinDateLocal;
            while (d < cycleStartLocal) {
                totalPastDays++;
                if (d.weekday === 7) {
                    totalPastSundays++;
                }
            ...
            // Get total present days (and half days) between joining date and cycle start
            const pastPresenceDocs = await StaffAttendance.find({
                staff: staffUser._id,
                date: { $gte: joinDateLocal.toFormat('yyyy-MM-dd'), $lt: cycleStart },
                status: { $in: ['present', 'half-day'] }
            }).lean();

            let totalPastPresent = 0;
            pastPresenceDocs.forEach(doc => {
                if (doc.status === 'half-day') {
                    totalPastPresent += 0.5;
                } else if (doc.status === 'present') {
                    totalPastPresent += 1;
                }
            });

            // Math: (जॉइनिंग से अब तक के कुल दिन) - (कुल Present दिन) - (कुल Sundays)
            let totalUsedLeaves = totalPastDays - totalPastPresent - totalPastSundays;
            if (totalUsedLeaves < 0) totalUsedLeaves = 0;
            
            let previousLeaveBalance = pastTotalQuota - totalUsedLeaves;
            if (previousLeaveBalance < 0) previousLeaveBalance = 0; // The Safety Rule (Zero Floor)
            totalLeaveAccrued = previousLeaveBalance;
        }

        const leavesTakenThisMonth = approvedLeaveDays + (halfDays * 0.5);
        const totalAvailable = totalLeaveAccrued + monthlyLeaveAllowance;
        
        const paidAbsences = Math.min(leavesTakenThisMonth, totalAvailable);
        
        if (leavesTakenThisMonth > totalAvailable) {
            extraLeaves = leavesTakenThisMonth - totalAvailable;
            deduction = extraLeaves * (baseSalary / divisor);
            availableLeave = 0;
        } else {
            availableLeave = totalAvailable - leavesTakenThisMonth;
        }

        earnedDays = presentDays + paidSundays + paidAbsences;
        finalSalary = (earnedDays / divisor) * baseSalary;
        
        // Export UI variables specifically mapped for the new layout
        totalLeaveUsed = leavesTakenThisMonth;
    }

    return {
        cycleStart,
        cycleEnd,
        totalDaysInCycle: totalDaysInCycle, // Keep actual days for UI display
        actualDaysInCycle: totalDaysInCycle,
        presentDays,
        halfDays,
        approvedLeaveDays,
        unapprovedAbsences,
        ...
        perDaySalary: Math.round(baseSalary / divisor),
