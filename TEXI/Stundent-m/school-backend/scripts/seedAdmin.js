...
const seed = async () => {
    try {
        ...
        if (!adminUser) {
            ...
            adminUser.password = '@2526Bigday'; // Force password reset so we know what it is
            if (targetCompany) {
                adminUser.company = targetCompany._id;
            }
            await adminUser.save();
            console.log('Master SuperAdmin user updated with correct role, company, and password reset');
        }
